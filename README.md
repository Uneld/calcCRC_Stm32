# CRC Calculation for STM32 use Python
...
- Совместимость с полиномом `0x04C11DB7` (IEEE 802.3).
- Поддержка обработки массивов байт (`uint8_t*`) и слов (`uint32_t*`).
- Результат ограничен до `uint8_t`, как в STM32 (младший байт регистра CRC).
- Возможность проверки входных данных на десктопе перед прошивкой в контроллер.
...
### STM32
Добавьте `test_crc.cpp` в проект CubeIDE и соберите.  
